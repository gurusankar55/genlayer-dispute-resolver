...
class DisputeResolver(gl.Contract):
    ...
    def submit_evidence(
        ...
        if dispute.status != "Open":
            raise gl.UserError(
                "evidence can only be submitted while Open"
            )
        ...
        evidence_id = u256(
            dispute_id * 1000000 + dispute.evidence_count
        )
    ...
    def _evidence_text(
        ...
        for index in range(dispute.evidence_count):
            ...
            parts.append(
                f"Evidence {index + 1}: "
                f"URI={item.uri}; notes={item.notes}"
            )
    ...
    def resolve(self, dispute_id: int) -> str:
        ...
        if dispute.status != "Voting":
            raise gl.UserError(
                "dispute must be in Voting state"
            )

        if now < dispute.vote_deadline:
            raise gl.UserError(
                "voting window has not ended"
            )
...
"""
        ...
        if not isinstance(
            ...
            raise gl.UserError(
                "consensus did not return a valid result"
            )
        ...
        if outcome not in (
            ...
            raise gl.UserError(
                "consensus produced an invalid outcome"
            )

        dispute.outcome = outcome

        dispute.resolution_reason = str(
            decision.get(
                "reason",
                "",
            )
    ...
    def challenge(
        ...
        if dispute.status != "Resolved":
            raise gl.UserError(
                "only a Resolved dispute can be challenged"
            )
        ...
        dispute.challenge_count = u32(
            dispute.challenge_count + 1
        )

        dispute.status = "Voting"

        dispute.vote_deadline = u64(
            now + 300
        )

        dispute.challenge_deadline = u64(
            now + 600
        )
    ...
    def finalize(self, dispute_id: int) -> None:
        ...
        if dispute.status != "Resolved":
            raise gl.UserError(
                "only a Resolved dispute can be finalized"
            )
